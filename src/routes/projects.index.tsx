...
export const Route = createFileRoute("/projects/")({
  ...
  head: () => ({
    meta: [
      { title: "Projects — Hyderabad Realty Choices" },
      {
        name: "description",
        content:
          "Explore premium residential projects across Hyderabad — filter by location, BHK, budget and status.",
      },
      { property: "og:title", content: "Projects — Hyderabad Realty Choices" },
      {
...
function ProjectsListingPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return list.filter((p) => {
      if (search.location && p.location?.slug !== search.location) return false;
      if (search.type && (p.property_type ?? "") !== search.type) return false;
      if (search.status && (p.status ?? "") !== search.status) return false;
      if (search.bhk) {
        const units = (p.unit_types ?? "").toString().toLowerCase();
        if (!units.includes(search.bhk.toLowerCase())) return false;
      }
      if (search.q) {
  ...
  const hasFilters = Boolean(
    search.location || search.q || search.type || search.bhk || search.status,
  );
  ...
  return (
    <PageShell
      eyebrow="Projects"
      title="Explore Premium Projects"
      subtitle="Filter across Hyderabad's most desired developments."
    >
      {/* Filter card */}
      <form
        onSubmit={(e) => {
      ...
      >
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-[1.2fr_1.4fr_1fr_1fr_1fr_auto]">
          <FieldSelect
            label="Location"
            name="location"
      ...
      {isError ? (
        ...
        <EmptyState
          ...
          description="Try widening your search — remove a filter or explore all developments."
          action={
